...
class SigmaEstimator:
    def __init__(
        self, name: str, feature_schema: Dict[str, Dict[str, str]], n_trials: int, cv: int = 3
    ):
        self.name = name
        self.quantile_model_16 = QuantileRegressionModel(
            f"{name}_quantile_16", feature_schema, n_trials, alpha=0.16, cv=cv
        )
        self.quantile_model_84 = QuantileRegressionModel(
            f"{name}_quantile_84", feature_schema, n_trials, alpha=0.84, cv=cv
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "meta": {"name": self.name, "model_type": "sigma_estimator", **self.metadata},
